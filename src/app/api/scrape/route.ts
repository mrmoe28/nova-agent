...
import { scrapeProductPage, scrapeMultipleProducts, detectCategory } from '@/lib/scraper'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (saveToDatabase && !distributorId) {
      return NextResponse.json(
        { success: false, error: 'distributorId is required when saveToDatabase is true' },
        { status: 400 }
      )
    }
