...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logger.info({ distributorCount: distributors.length }, 'Found active distributors')
    ...
    for (const distributor of distributors) {
      ...
      logger.info(
        { distributorId: distributor.id, distributorName: distributor.name, crawlJobId: crawlJob.id },
      ...
      try {
        ...
        logger.info({ distributorId: distributor.id, productsScraped: scrapedProducts.length }, 'Products scraped')
          ...
          try {
            ...
            if (existingEquipment) {
              ...
              await prisma.equipment.update({
                ...
                data: {
                  name: product.name,
                  manufacturer: product.manufacturer || existingEquipment.manufacturer,
                  description: product.description || existingEquipment.description,
              ...
              if (Math.abs(existingEquipment.unitPrice - product.price) > 0.01) {
        ...
        await prisma.scrapeHistory.create({
          data: {
            ...
            status: productsUpdated > 0 ? 'success' : scrapedProducts.length > 0 ? 'partial' : 'failed',
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Scheduled scraping failed',
        duration,
      },
      { status: 500 }
    )
